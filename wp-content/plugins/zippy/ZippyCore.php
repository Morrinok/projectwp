...
class ZippyCore
{
    ...
    {
        return apply_filters('zippy-protected-meta-keys', ['_edit_last', '_edit_lock', '_revision-control']);
    }
    ...
    public function adminInit()
    {
        ...
        if (isset($_REQUEST['__action']) && $_REQUEST['__action'] === 'zippy-unzip') {
            ...
            $options = [
                ...
                'importCategories' => isset($_REQUEST['categories']) && $_REQUEST['categories'] === 'on',
    ...
    public static function unzipPosts($pathToArchive, $options = [], $deprecated = '')
    {
        ...
        try {
            ...
            foreach ($postsData as $data) {
                ...
                if ($options['importMeta'] && !empty($data['post_meta'])) {
                    ...
                    foreach ($meta as $metaKey => $metaValue) {
                        update_post_meta(
                            $postId,
                            $metaKey,
                            self::replaceURLs(maybe_unserialize($metaValue), $archiveBaseUrl)
                        );
                    }
                }
                ...
                if ($options['importCategories'] && !empty($data['post_taxonomies'])) {
                ...
                if ($options['importMedia']) {
                    ...
                    if (!empty($data['attachments'])) {
                        ...
                        foreach ($attachments as $attachment) {
                            ...
                            $matchedAttachmentId = $wpdb->get_var($wpdb->prepare("SELECT ID 
                                                                              FROM $wpdb->posts 
    ...
    private function getZippyForm()
    {
        ...
        return '<form action="" method="post" enctype="multipart/form-data" autocomplete="off">' .
               ...
               '<div><label for="zippyCategories"><input id="zippyCategories" type="checkbox" name="categories" checked="checked" /> ' . __('Import categories, taxonomies', 'zippy') . '</label></div>' .
